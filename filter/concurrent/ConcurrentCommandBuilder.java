...
public class ConcurrentCommandBuilder {
	...
	public static ConcurrentFilter createFiltersFromCommand(String command) {
		...
		// for all the commands, split them by pipes, construct each filter, and add
		// them to the filters list.
		...
		if (removeRedir.length > 1) {
			...
			if (removeRedir[1].contains("|")) {
				System.out.printf(Message.CANNOT_HAVE_OUTPUT.toString(),
						">" + removeRedir[1].substring(0, removeRedir[1].indexOf("|")));
				return null;
			}
	...
	private static boolean linkFilters(List<ConcurrentFilter> filters, String command) {
		...
		while (iter.hasNext()) {
			...
			// additional checks
			if (curr instanceof CdFilter || curr instanceof HeadFilter || curr instanceof LsFilter
					|| curr instanceof PwdFilter) {
				System.out.printf(Message.CANNOT_HAVE_INPUT.toString(), cmdlist[cmdindex].trim());
				// we need to close any file input streams opened by head whenever it is not
				// executed
				// due to error while linking filters
				for (ConcurrentFilter f : filters) {
					if (f instanceof HeadFilter) {
						((HeadFilter) f).terminate();
					}
				}
				return false;
			}
